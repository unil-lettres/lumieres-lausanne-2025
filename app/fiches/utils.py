...
def get_last_model_activity(model_instance):
    ...
    try:
        act = ActivityLog.objects.filter(
            model_name=model_instance.__class__.__name__, object_id=model_instance.pk
        ).latest("date")
...
def get_grouped_objet_activities(obj):
    ...
    grouped_activities = [tuple(g[1])[0] for g in groupby(activities, lambda x: f"{x.user.id}{x.date.date()}")]
...
def user_can_change_documentfile(user, docfile):
    ...
    Ownership or the broad ``change_any_documentfile`` permission grants access.
    """
    if not getattr(user, "is_authenticated", False):
        return False
    return user.has_perm("fiches.change_any_documentfile") or getattr(
        docfile, "access_owner_id", None
...
def user_can_delete_documentfile(user, docfile):
    ...
    Ownership or the broad ``delete_any_documentfile`` permission grants access.
    """
    if not getattr(user, "is_authenticated", False):
        return False
    return user.has_perm("fiches.delete_any_documentfile") or getattr(
        docfile, "access_owner_id", None
    ) == getattr(user, "id", None)


def user_can_delete_biblio(user, biblio):
    """Return whether the user may delete this bibliography under the ownership policy."""
    if not getattr(user, "is_authenticated", False) or not user.has_perm("fiches.delete_biblio"):
        return False
    return user.has_perm("fiches.delete_any_biblio") or getattr(biblio, "creator_id", None) == getattr(
        user, "id", None
    )


def user_can_delete_transcription(user, transcription):
    """Return whether the user may delete this transcription under the ownership policy."""
    if not getattr(user, "is_authenticated", False) or not user.has_perm("fiches.delete_transcription"):
        return False
    if user.has_perm("fiches.delete_any_transcription"):
        return True
    user_id = getattr(user, "id", None)
    return user_id in {
        getattr(transcription, "access_owner_id", None),
        getattr(transcription, "author_id", None),
    }
...
def get_default_publisher_user():
    ...
    return User.objects.filter(first_name__icontains="bea").order_by("id").first()
