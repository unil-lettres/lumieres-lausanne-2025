...
def extract_all_markers(text):
    ...
    for m in re.finditer(pattern1, text):
        markers.append(
            {"pattern": m.group(0), "type": "p-format", "page": int(m.group(1))}
        )
    ...
    pattern4 = r"(?:<|&lt;)(\d+)(?:>|&gt;)"
    explicit_angle_positions = set(
        m.span() for m in re.finditer(pattern2, text, re.IGNORECASE)
    )
    ...
    pattern5 = r"\[(\d{1,3})\]"
    explicit_bracket_positions = set(
        m.span() for m in re.finditer(pattern3, text, re.IGNORECASE)
    )
...
def analyze_patterns():
    ...
    print(
        f"Transcriptions with pagination markers: {len(trans_with_markers)} ({len(trans_with_markers) / total * 100:.1f}%)"
    )
    print(
        f"Transcriptions without markers: {len(trans_without_markers)} ({len(trans_without_markers) / total * 100:.1f}%)"
    )
    ...
    if mixed_transcriptions:
        ...
        for item in mixed_transcriptions[:10]:
            ...
            print(
                f"  Transcription {item['id']}: {item['count']} markers ({types_str})"
            )
    else:
    ...
    first_pages = [
        item["first_page"] for item in trans_with_markers if item["first_page"]
    ]
